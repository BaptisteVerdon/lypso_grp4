...
<article>
    <table class="table table-bordered">
        ...
        foreach($years as $year){
            ...
            foreach ($status as $statu){
                echo '<td class="text-center">'.dayOffData_countStatuFromYear($statu['name'],$year['year'],$user_id).'</td>';
            }
            echo '<td class="text-center"><strong>'.dayOffData_countAllStatuFromYear($year['year'],$user_id).'</strong></td>';
            echo '</tr>';
        }
        ?>
        <tr>
            ...
            <?php
            foreach ($status as $statu){
                echo'<td class="text-center"><strong><h1>'.dayOffData_countStatu($statu['name'],$user_id).'</strong></h1></td>';
            }
