...
<article>
    ...
    <form method="POST" action=".?control=dayOff&action=directorUserSummary">
        <label>
        ...
        <select name="user_id" id="user_id-selector"  >
            ...
            foreach ($users as $user){
                if ( ! (userData_getNameRoleFromUserId($user['role_id']) == "admin")){
    ...
    if ( ! ($chosenUserId == "all")){
        ...
        if (userData_getNameRoleFromUserId($chosenUserId) == 'director' ){
            echo 'Directeur'.'<br>';
        }elseif (userData_getNameRoleFromUserId($chosenUserId) == 'manager' ){
            echo 'Manager'.'<br>';
